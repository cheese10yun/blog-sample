...
HTTP Client는 다른 라이브러리보다 교체를 자주 하는 편에속합니다. 변경될 가능성이 높은 만큼 라이브로리 교체시에도 그 영향 범위도 적게 설계하는 것이 바람직합니다. 예를 들어 `RestTemplate`에서는 HTTP Status Code를 포함하여 요청이 성공적이었는지, 오류가 발생했는지 등을 판단할 수 있는 `ResponseEntity<T>`객체를 리턴타입으로 사용하면 라이브러리의 의존도가 지나치게 높아집니다.
