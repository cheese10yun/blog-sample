...
1. **쿼리 최적화**:
    - `activeConnections`가 빠르게 응답하지 못하는 이유가 데이터베이스 쿼리의 성능에 있다면, 쿼리를 최적화하여 응답 속도를 높이는 것이 우선입니다. 쿼리 실행 시간이 줄어들면 커넥션이 더 빠르게 반환되기 때문에 대기 중인 요청이 커넥션을 더 빨리 할당받을 수 있습니다.
2. **connection-timeout 시간 조정**:
