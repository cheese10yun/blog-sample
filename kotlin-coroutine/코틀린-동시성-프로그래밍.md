...
코루틴에서 명령이 중첩되는 것은 counter++ 작업이 원자적이지 않기 떄문이다. 실제로 이 작업은 counter의 현재 값을 읽은 당므 그 값을 1씩 증가시킨 후에 그 결과를 counter에 다시 저장하는 세가지 명령어로 나눌 수 있다. counter++에서 원자성이 없기 때문에 코루틴이 다른 코루틴이 하는 조작을 무시하고 값을 일고 수정할 수 있다.
