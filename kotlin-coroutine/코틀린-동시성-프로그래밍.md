...
* 다른 스레드가 수정한 후 스레드의 읽기가 발생하지만, 스레드 로컬 캐시가 업데이트 되지 않을 때: 스레드는 로컬 캐시가 제떄 업데이트 되지 않아서 다른 스레드가 Y로 설정 한 후에 카운터의 값을 X로 익을 수 있다. 최종 결과는 비슷하다. 두 번째 스레드는 카운터의 값을 증가시키지만 오래된 값으로 시작 했기 때문에 이미 변경한 내여용을 덮어 쓴다.

짐작할 수 있지만 휩발성 변수는 상태를 읽을 때 항상 최신 값을 유지함으로써 두 번째 경우 보호 기능을 제공한다. 그러나 두 스레드가 여전히 같은 값으로 수정되도록 하기 떄문에 충분히 가까운 현재 값을 읽을 수 있다. 데이터 유실이 발생하기 때문에 첫 번쨰 경우는 보호 기능을 보장하지 않는다.
...
반면에 두 번쨰 전제는 휘발성 변수의 의존성 떄문에 일관성 없는 상태가 생성되는 것을 피할 수 있도록 도와준다. 예는 두 번째 전제를 따르지 않기 때문에 안전하다.
