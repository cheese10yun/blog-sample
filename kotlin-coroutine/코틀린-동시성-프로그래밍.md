...
각 인덱서가 응답을 반환하는 시간이 다르고 일부는 상당히 오래 걸릴 수 있다는 문제점이 있다. 그렇기 떄뭉네 모른 코루틴이 완료 될 때가지 기다려야 하는데, 결과를 표시를 지연시켜서 사용자가 즉시 결과와 상호작용하는 것을 방해한다.
...
채널의 작동 방식 떄문에 코루틴이 채널로 부터 가져온 데이터를 처리하는 작업이 가능해지면서 부하 분산에 대해 걱정 할필요가 없어진 것이다.
...
채널이 열려 있고 가득 차 있지 않은 떄는 offer()는 요소를 대기열에 추가한다. 일시 중단 연산에서 발생하지 않은 채널에 요소를 추가하는 유일한 방법이다. 
