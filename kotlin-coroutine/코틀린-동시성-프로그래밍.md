...
getProfile을 실행을 시작한 후 곧 바로 fetchProfile이 실행될 때까지 대기한다. fetchProfile이 응답 처리된 후 getProfile은 한 번 더 일시 중단되는데, calculcateAge이 호출되기 떄문이다. calculateAge가 완료되면 getProfil의 실행이 완료될 때까지 계속 실행된다.
