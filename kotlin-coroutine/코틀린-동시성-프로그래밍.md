...
#### 스레드 풀
스레드 풀을 갖고 있으면 해당 풀에서 가용한 스레드에서 코루틴을 시작하고 재개한다. 런타임이 가용한 스레드를 정하고 부하 분산ㅇ늘 위한 방법도 정하기 떄문에, 따로 할 작업은 없다.
...
코루틴이 취소될 때까지 0.5초마다 "still running"을 출력한다. 코루틴을 취소할 떄는 finally 블록이 실행된다. 메인 스레드가 1.2s 동안 일시 중지된 다음 작업을 취소하고 애플리케이션 종료 전에 전체 실행 시간을 출력 한다.
