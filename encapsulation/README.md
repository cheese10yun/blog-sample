...
객체지향 개념에서 캡슐화는 정말 중요한 개념이라고 생각합니다. 캡슐화를 잘 지켜야 클래스 간의 결합도를 낮출 수 있습니다. 이것은 지속해서 코드를 유지 보수하기 위해서는 반드시 필요하다고 생각합니다. 

이미 수많은 책들이 이 개념에 관해서 설명을 하고 있지만 글 읽는 것은 이해되지만 정작 캡슐화 좋은 코드를 작성하는 것은 또 다른 과제였습니다. 
...
테스트 코드는 수많은 장점이 있습니다. 저는 이 중에서 지금 설명해 드리는 장점 또 한 테스트 코드의 엄청난 장점이라고 생각합니다.

> 문제 있는 코드를 빨리 파악할 수 있다.

해당 기능의 테스트 코드를 작성하면 이 코드의 문제점을 가장 빠르게 파악할 수 있습니다. 저는 저 코드가 왜 캡슐화에 안 좋은 코드인지 테스트 코드를 통해서 알게 됐습니다. 위에서 언급한 1~5 문제들을 테스트 코드 작성 시 파악했고 리팩토링 작업을 진행했습니다.
...
* 이는 재사용성이 높아졌습니다. 만약 상품 등록이 성공했을 경우 메시지 플랫폼을 통해서 응답받고 싶다면 Product에서 Message 객체를 선언하기만 하면 됩니다.
