...
그러나 이러한 방식은 **단순히 도큐먼트 객체(`T`) 타입**을 기준으로 설계되어, **복잡한 데이터 변환**, **조인**, **그룹화** 등 다양한 데이터 처리 작업을 다루는 데 한계가 있었습니다. 특히, **프로젝션**을 활용한 **데이터 조회**나 **복잡한 쿼리**는 `Aggregation`을 사용하여 해결해야 하기 때문에, **`Aggregation`을 기반으로 하는 페이징 처리**가 필요합니다.  
