...
본 포스팅은 Spring Data MongoDB에서 발생하는 N+1 문제를 중심으로, 성능 관점에서의 문제점과 이를 개선하기 위한 최적화 전략을 상세하게 다루고자 합니다. 특히, 연관 데이터를 로딩할 때 불필요한 추가 조회로 인한 성능 저하 문제를 해결하기 위해 `@DBRef`의 lazy 옵션 활용, `$lookup` 기반의 Aggregation 쿼리, 그리고 단순 값 객체 기반의 연관관계 모델링 등 여러 접근 방식을 비교 분석합니다.
