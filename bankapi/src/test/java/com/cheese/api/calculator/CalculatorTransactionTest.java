...
public class CalculatorTransactionTest {
    ...
    @Test
    public void Transaction_Dto_Test() {
        final CalculatorDto.Remittance remittance = getSendMoney();
        final CalculatorDto.Deposit deposit = getReceiveMoney();
        final CalculatorDto.Transaction transaction = CalculatorDto.Transaction.builder()
                .remittance(remittance)
                .deposit(deposit)
                .build();

        assertThat(transaction.getRemittance().getMoney().getAmount(), is(REMITTANCE_AMOUNT));
