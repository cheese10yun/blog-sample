...
| NO | 상호명   | 사업자 번호 | 파트너 사업자 상태 | 가맹점 상태 | 최종 상태 |
| -- | :------- | ----------- | ------------------ | :---------- | :-------- |
| 1  | (주)테크 | 111111111   | 오픈               | 휴업        | 오픈      |
| 2  | (주)가구 | 111111112   | 오픈               | 오픈        | 오픈      |
| 3  | (주)한용 | 111111113   | 휴업               | 오픈        | 휴업      |
| 4  | (주)가족 | 111111114   | 휴업               | 오픈        | 휴업      |
| 5  | (주)손신 | 111111115   | 폐업               | 폐압        | 폐업      |
| 6  | (주)지구 | 111111116   | 폐업               | 폐업        | 폐업      |

파트너 서비스 서버에서 응답하는 사업자 상태, 현재 가맹점 상태, 그에 따른 최종 상태를 고려하여 여러 테스트 케이스를 대응하도록 테스트 코드를 작성하는 것이 중요합니다. 그러기 위해서는 다양한 테스트 케이스에 대응하는 데이터 셋업 즉, Given을 잘 작성하는 것이 중요합니다.
...
```
ClientTestConfiguration에서 설정한 mockPartnerClient 객체로 Partner 사업자 정보에 대한 Mocking을 gvien 함수로 작성합니다. 이렇듯 해당 비즈니스 로직의 모든 케이스에 대해서 폭넓은 테스트 코드로 커버하는 것이 중요하며, 폭넓은 테스트를 하기 위해서는 HTTP Mocking 보다 행위 자체를 Mocking 하는 것이 훨씬 더 효율 적입니다. 실제 테스트 코드도 모든 테스트 케이스에 대해서 정상적으로 변경됐는지 검증하고 있습니다.
