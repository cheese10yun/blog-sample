...
앞서 소개한 `QueryDslNoOffsetPagingReader`도 커서 기반 발상을 활용하지만, 그 용도는 **배치 Item Reader**에 특화되어 있습니다. 데이터를 처음부터 끝까지 한 방향으로 순차 처리하는 것이 목적이기 때문에 "다음 청크"만 지원하면 충분합니다.
...
클라이언트는 응답으로 받은 `nextCursor` 또는 `prevCursor` 값을 다음 요청의 `cursorKey`로 전달하는 방식으로 탐색을 이어갑니다. 서버는 커서 값과 방향(direction)만으로 다음 조회 범위를 결정하기 때문에, 전체 데이터 크기나 현재 페이지 번호를 유지할 필요가 없습니다.
