...
모든 경우에 사용할 수 있는 것은 아니다. 엔티티가 영속 상태가 디려면 식별자가 꼭 필요하다. **그런데 IDENTITY 식별자 생성 전략은 엔티티를 데이터베이스에 저장해야 식별자를 구할 수 있으므로 em.persist()를 호출하는 즉시 INSERT SQL이 데이터베이스에 전달된다. 따라서 쓰지 지연을 활용한 성능 최적화를 할 수가 없다.**
